    ...
    <div className="w-full py-6 flex flex-col gap-5">
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-[17px] font-semibold text-zinc-900 tracking-tight capitalize">
              ...
              <SwiperSlide key={index}>
                ...
                >
                  <div className="flex flex-col">
                    <span className={`text-[10px] font-semibold uppercase tracking-widest mb-0.5 ${isToday ? 'text-zinc-400' : 'text-zinc-500'}`}>
                      {format(day, "eee", { locale: es })}
                    </span>
                    <span className={`text-xl font-bold tracking-tight ${isToday ? 'text-white' : 'text-zinc-900'}`}>
                      {format(day, "d")}
                    </span>
                  </div>
                  {/* Indicador de que hay reservas ese dia (mock) */}
                  {Math.random() > 0.4 && (
                    <div className={`w-1.5 h-1.5 rounded-full ${isToday ? 'bg-emerald-400' : 'bg-zinc-300 group-hover:bg-zinc-400'}`} />
                  )}
                </button>
              </SwiperSlide>
            );
