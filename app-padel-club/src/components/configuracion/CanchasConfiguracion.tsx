import { useClubStore } from '@/src/store';
import { LayoutGrid, Plus, Trash2 } from 'lucide-react';
import React, { useState } from 'react'

export const CanchasConfiguracion = () => {
    const {canchas, addCancha, updateCancha, deleteCancha} = useClubStore();
    const [newCanchaName, setNewCanchaName] = useState("");
  const [newCanchaPrice, setNewCanchaPrice] = useState(20000);
      const handleAddCancha = (e: React.FormEvent) => {
        e.preventDefault();
        if (newCanchaName.trim()) {
          addCancha(newCanchaName, newCanchaPrice);
          setNewCanchaName("");
        }
      };
    ...
    <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            {canchas.map((cancha) => (
              <div 
                key={cancha.id} 
                className="bg-white p-5 rounded-2xl border border-zinc-200 shadow-sm hover:border-zinc-300 transition-all flex flex-col gap-4 group relative"
              >
            ...
            <form 
              ...
              onClick={() => { if(newCanchaName === "") document.getElementById('new-cancha-name')?.focus() }}
            >
